...
record.prototype.translate = function(item) {
	...
	if ( (this.getFieldSubfields("200")[0]) && (!(this.getFieldSubfields("245")[0])) )
	{
		...
		var tempLang=this.getFieldSubfields("101")[0]['a'];
		if (tempLang) {
			if ((tempLang.indexOf("فارسی")>=0) || (tempLang.indexOf("Per")>=0) || 
				(tempLang.indexOf("per")>=0) || (tempLang.indexOf("fa")>=0)) {
				item.language="fa";
			} else if ((tempLang.indexOf("eng")>=0)) {
				item.language="en";
			} else {
				item.language=tempLang;
			}
		} else {
			item.language="en";
		}
